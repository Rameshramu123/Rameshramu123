...
def predict_disease():
    try:
        ...
        search_label = predicted_label.replace("_", " ")
        search_label = search_label.split(" ")[1:]
        search_label = " ".join(search_label).strip()

        try:
            disease_info = wikipedia.summary(search_label, sentences=5)
            wiki_url = wikipedia.page(search_label).url
        except Exception:
            disease_info = f"No additional info available for {search_label}."
            wiki_url = None
...
# ------------------------------
# NEW: CITY → LAT/LON ENDPOINT  ✅ (Added only)
# ------------------------------
from geopy.geocoders import Nominatim
geolocator = Nominatim(user_agent="krushimitra")

@app.route("/geocode-city", methods=["POST"])
def geocode_city():
    data = request.get_json()
    city = data.get("city")
